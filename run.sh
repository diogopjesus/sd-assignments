...
if [ "$1" = "clean" ]; then
    rm -r $SRC_PATH/*/*.class 2> /dev/null
    rm -r $SRC_PATH/*/*/*.class 2> /dev/null
    rm -r $LOG_PATH/ 2> /dev/null
    rm -r $DOC_PATH/ 2> /dev/null
    rm -r $BUILD_PATH/ 2> /dev/null
    rm -r $TEST_PATH/ 2> /dev/null
    exit 0;
fi
...
# # compile project
# javac --release 8 -cp .:$PREVIOUS_DIR/lib/genclass.jar **/*.java
# if [ "$?" -ne 0 ]; then
