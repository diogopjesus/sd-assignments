    ...
    try {
      while (!end)
        synchronized (Class.forName("serverSide.main.ServerHeistToTheMuseumGeneralRepository")) {
          try {
            (Class.forName("serverSide.main.ServerHeistToTheMuseumGeneralRepository")).wait();
          } catch (InterruptedException e) {
            GenericIO.writelnString("General Repository main thread was interrupted!");
          }
        }
    } catch (ClassNotFoundException e) {
      GenericIO.writelnString("The data type ServerHeistToTheMuseumGeneralRepository was not found (blocking)!");
      e.printStackTrace();
      System.exit(1);
    }
